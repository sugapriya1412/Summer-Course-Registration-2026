...
{	
	...
	public List<String> getEligibleCompulsoryCourseList(String semesterSubId, Integer progGroupId, Integer studentBatch, 
								Integer progSpecId, String registerNumber, String progSpecCode, Integer costCenterId, 
		...
		if (!tempObjectList.isEmpty())
		{
			for(Object[] e : tempObjectList)
			{
				...
				else if (preReqType == 8)
				{
					...
					{
						pcmbStatus = semesterMasterService.getPCMBStatusFromAdmissionsByRegisterNumber(registerNumber);
					}
				...
				if (compCheckFlag == 1)
				{
					...
					compCheckFlag = checkPreReqAndAntiReqStatus(semesterSubId,1, registerNumber, preRequisite);
					//LOGGER.trace("\n preRequisite Status: "+ compCheckFlag);
					
					//Checking Anti-Requisite
					if (compCheckFlag == 1)
					{
						compCheckFlag = checkPreReqAndAntiReqStatus(semesterSubId, 2, registerNumber, antiRequisite);
	...
	public int checkPreReqAndAntiReqStatus(String semesterSubId, int requisiteType, String registerNumber, String requisiteCourse)
	{
		...
		else
		{
			...
			regCourseCodeList = courseRegistrationService.getByRegisterNumberAndCourseCodeForPARequisite(semesterSubId, registerNumber, courseCodeList);
