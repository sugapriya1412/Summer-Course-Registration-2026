...
{	
	@Query("select a from StudentHistoryModel a where a.studentHistoryPKId.registerNumber =?1 and "+
	...
	List<StudentHistoryModel> findStudentHistoryPARequisite(String registerNumber, String[] courseCode);
	...
	List<StudentHistoryModel> findStudentHistoryPARequisite2(String registerNumber, List<String> courseCode);


	@Query(value="select COURSE_CATALOG_COURSE_ID, COURSE_CODE, COURSE_TITLE, CRSTYPCMPNTMASTER_COURSE_TYPE, "+
			"to_char(EXAM_MONTH,'DD-MON-YYYY') as exam_date from academics.STUDENT_HISTORY where "+
			"STDNTSLGNDTLS_REGISTER_NUMBER =?1 and COURSE_CODE not in (?2) and "+
			"GRADE in ('Fail','F','N','N1','N2','N3','N4') and CRSTYPCMPNTMASTER_COURSE_TYPE "+
			"not in ('ETH','ELA','EPJ','PJT','SS','OC','ECA') and COURSE_CODE not in "+
			"(select distinct COURSE_CODE from academics.STUDENT_HISTORY where STDNTSLGNDTLS_REGISTER_NUMBER =?1 "+
			" and GRADE in ('S','A','B','C','D','E','U','P','Pass')) and COURSE_CODE not in "+
	...
	List<Object[]> findStudentHistoryCS2(String registerNumber, List<String> courseCode);


	@Query(value="select COURSE_CATALOG_COURSE_ID, COURSE_CODE, COURSE_TITLE, CRSTYPCMPNTMASTER_COURSE_TYPE, "+
			"to_char(EXAM_MONTH,'DD-MON-YYYY') as exam_date from academics.STUDENT_HISTORY where "+
			"STDNTSLGNDTLS_REGISTER_NUMBER =?1 and COURSE_CODE not in (?2) and GRADE in ('Fail','F','N','N1','N2','N3','N4') "+
			"and CRSTYPCMPNTMASTER_COURSE_TYPE not in ('ETH','ELA','EPJ','PJT','SS','OC','ECA') and COURSE_CODE not in "+
			"(select distinct COURSE_CODE from academics.STUDENT_HISTORY where STDNTSLGNDTLS_REGISTER_NUMBER "+
			"=?1 and GRADE in ('S','A','B','C','D','E','U','P','Pass')) and COURSE_CODE not in "+
			"(select EQUIVALENT_COURSE_CODE from academics.COURSE_EQUIVALANCES where COURSE_CODE in (?2)) "+
			"and COURSE_CODE not in (select COURSE_CODE from academics.COURSE_EQUIVALANCES where "+ 
			"EQUIVALENT_COURSE_CODE in (?2)) and COURSE_CODE not in "+
			...
			"a.COURSE_BASKET_ID=b.COURSE_ID) and COURSE_CODE not in "+
			...
			"b.COURSE_CATALOG_COURSE_ID=c.COURSE_ID) order by COURSE_CODE", nativeQuery=true)
	List<Object[]> findStudentHistoryCS3(String registerNumber, List<String> courseCode, Integer specializationId, 
			Integer studentYear, Float curriculumVersion);


	@Query(value="select COURSE_CATALOG_COURSE_ID, COURSE_CODE, COURSE_TITLE, CRSTYPCMPNTMASTER_COURSE_TYPE, "+
			"to_char(EXAM_MONTH,'DD-MON-YYYY') as exam_date from academics.STUDENT_HISTORY where "+
			"STDNTSLGNDTLS_REGISTER_NUMBER =?1 and COURSE_CODE not in (?2) and GRADE "+
			"in ('Fail','F','N','N1','N2','N3','N4') and CRSTYPCMPNTMASTER_COURSE_TYPE "+
			"not in ('ETH','ELA','EPJ','PJT','SS','OC','ECA') and COURSE_CODE not in "+
			"(select distinct COURSE_CODE from academics.STUDENT_HISTORY where STDNTSLGNDTLS_REGISTER_NUMBER =?1 "+
			" and GRADE in ('S','A','B','C','D','E','U','P','Pass')) and COURSE_CODE not in "+
			"(select EQUIVALENT_COURSE_CODE from academics.COURSE_EQUIVALANCES where COURSE_CODE in (?2)) "+
			...
			"COURSE_CODE not in (select c.CODE from ACADEMICS.PRG_SPLZTN_CURRICULUM_DETAILS a, "+
	...
	List<Object[]> findStudentHistoryCS4(String registerNumber, List<String> courseCode, 
	...
	@Query(value="select COURSE_CATALOG_COURSE_ID, COURSE_CODE, COURSE_TITLE, CRSTYPCMPNTMASTER_COURSE_TYPE, "+
			...
			"(select EQUIVALENT_COURSE_CODE from academics.COURSE_EQUIVALANCES where COURSE_CODE in (?2)) "+
			"and COURSE_CODE not in (select COURSE_CODE from academics.COURSE_EQUIVALANCES where "+
			"EQUIVALENT_COURSE_CODE in (?2)) and COURSE_CODE in "+
			"(select c.CODE from ACADEMICS.PRG_SPLZTN_CURRICULUM_DETAILS a, ACADEMICS.BASKET_COURSE_CATALOG b, "+
			"ACADEMICS.COURSE_CATALOG c where a.PRGSPLZN_PRG_SPECIALIZATION_ID=?3 and a.ADMISSION_YEAR=?4 "+
			...
			"b.COURSE_CATALOG_COURSE_ID=c.COURSE_ID) order by COURSE_CODE", nativeQuery=true)
	List<Object[]> findCSCourseByCourseCategoryAndBasketId(String registerNumber, List<String> courseCode, 
	...
	@Query("select sum(a.credit) as FCredits from StudentHistoryModel a where (a.studentHistoryPKId.registerNumber "+
			" = ?1) and a.grade in ('Fail','F','N','N1') and a.courseTypeComponentModel.component in (1,3)")
	Integer findStudentHistoryFailCourseCredits2(String registerNumber);
	...
	@Query(value="select a.GRADE, a.COURSE_CATALOG_COURSE_ID, a.COURSE_CODE, a.CRSTYPCMPNTMASTER_COURSE_TYPE "+
			...
			"academics.COURSE_TYPE_COMPONENT_MASTER b where a.STDNTSLGNDTLS_REGISTER_NUMBER =?1 and "+
	...
	List<Object[]> findStudentHistoryGrade3(String registerNumber, String courseCode);
	...
	List<Object[]> findStudentHistoryCEGrade4(String registerNumber, String courseCode);

	@Query(value="select a.GRADE, a.COURSE_CATALOG_COURSE_ID, a.COURSE_CODE, a.CRSTYPCMPNTMASTER_COURSE_TYPE "+
			"as GEN_COURSE_TYPE from academics.STUDENT_HISTORY a, academics.COURSE_TYPE_COMPONENT_MASTER b "+
			"where a.STDNTSLGNDTLS_REGISTER_NUMBER =?1 and a.GRADE in ('Fail','F','Y','N','N1','N2','N3','N4') "+
			"and a.CRSTYPCMPNTMASTER_COURSE_TYPE=b.COURSE_TYPE order by a.COURSE_CODE", 
			nativeQuery=true)
	List<Object[]> findStudentHistoryGIAndFailCourse(String registerNumber);

	@Query(value="select distinct a.CRSTYPCMPNTMASTER_COURSE_TYPE from academics.STUDENT_HISTORY a, "+
			"academics.COURSE_TYPE_COMPONENT_MASTER b where a.STDNTSLGNDTLS_REGISTER_NUMBER =?1 "+
	...
	List<String> findStudentHistoryFailComponentCourseType(String registerNumber, String courseId, 
			String examMonth);
	...
	@Query(value="select a.GRADE, a.COURSE_CATALOG_COURSE_ID, a.COURSE_CODE, a.CRSTYPCMPNTMASTER_COURSE_TYPE as GEN_COURSE_TYPE "+
			...
			"=?1 and a.COURSE_CATALOG_COURSE_ID=?2 and to_char(EXAM_MONTH,'DD-MON-YYYY')=?3 and a.GRADE in ('N2','N4') "+
			"and a.CRSTYPCMPNTMASTER_COURSE_TYPE=b.COURSE_TYPE order by a.CRSTYPCMPNTMASTER_COURSE_TYPE desc", nativeQuery=true)
	List<Object[]> findStudentHistoryNotAllowedGrade(String registerNumber, String courseId, String examMonth);

	@Query(value="select a.SEMESTER_SUB_ID, a.semester_desc, a.REGISTRATION_TYPE, (case "+ 
			"when (a.REGISTRATION_TYPE = 'RAR') then 'Regular Arrear' when (a.REGISTRATION_TYPE in ('ME_FAT')) then 'Makeup Exam' when (a.REGISTRATION_TYPE = 'RAR_RFAT') "+
			"then 'Regular Arrear' end) as reg_type_desc, a.course_id, a.course_code, a.course_title, a.course_type, "+ 
			"a.START_DATE, (case when (a.REGISTRATION_TYPE = 'RAR') then 3 when (a.REGISTRATION_TYPE = 'RAR_RFAT') then 4 else 5 end) "+ 
			"as reg_type_no from ("+  
			...
			"from ACADEMICS.SEMESTER_DETAILS a, EXAMINATIONS.ARREAR_COURSE_ALLOCATION_RAR b, EXAMINATIONS.ARREAR_COURSE_REGISTRATION_RAR c, "+ 
			"ACADEMICS.COURSE_CATALOG d where a.SEMESTER_SUB_ID=b.SEMSTR_DETAILS_SEMESTER_SUB_ID and a.SEMESTER_SUB_ID<>?1 and c.STDNTSLGNDTLS_REGISTER_NUMBER =?2 "+ 
			"and c.REGISTRATION_STATUS=2 and c.REGISTRATION_TYPE not in ('RAR_RFAT') and (c.result_status is null or c.result_status not in (2)) and "+
			...
			"c.COURSE_CATALOG_COURSE_ID=d.COURSE_ID and d.CODE=?3)"+ 
			"union all "+ 
			"(select a.SEMESTER_SUB_ID, a.DESCRIPTION as semester_desc, c.REGISTRATION_TYPE, b.COURSE_CATALOG_COURSE_ID as course_id, "+ 
			"d.CODE as course_code, d.TITLE as course_title, b.CRSTYPCMPNTMASTER_COURSE_TYPE as course_type, a.START_DATE "+ 
			"from ACADEMICS.SEMESTER_DETAILS a, EXAMINATIONS.ARREAR_COURSE_ALLOCATION_RAR b, EXAMINATIONS.ARREAR_COURSE_REGISTRATION_RAR c, "+ 
	...
	List<Object[]> findArrearRegistrationWithCEByRegisterNumberAndCourseCode(String semesterSubId, String registerNumber, String courseCode);
	...
	@Query(value="select PRE_CRSCATALOG_COURSE_ID, PRE_CRSCATALOG_COURSE_CODE, COURSE_CHANGE_OPTION from "+
			"EXAMINATIONS.STUDENT_COURSE_CHANGE_HISTORY where STDNTSLGNDTLS_REGISTER_NUMBER =?1 "+
			"and PRE_CRSCATALOG_COURSE_CODE=?2 and COURSE_CHANGE_OPTION not in ('CVCC','CVNN')", 
			nativeQuery=true)
	List<Object[]> findCourseChangeHistoryByRegisterNumberAndCourseCode3(String registerNumber, 
			String courseCode);
	...
	@Query(value="select distinct b.COURSE_ID from RESEARCH.COURSEWORK_OPTION_ALLOCATION a, ACADEMICS.COURSE_CATALOG b "+
			"where a.STDNTSLGNDTLS_REGISTER_NUMBER=?1 and a.COURSE_TYPE='RHR' and "+
			"a.COURSE_CATALOG_COURSE_ID=b.COURSE_ID order by b.COURSE_ID ", nativeQuery=true)
	List<String> findRPCourseWorkCourseIdByRegisterNumber(String registerNumber);
	...
	@Query(value="select a.COURSE_CATALOG_COURSE_ID, a.COURSE_CODE, a.CRSTYPCMPNTMASTER_COURSE_TYPE as generic_course_type, b.DESCRIPTION "+
			...
			"ACADEMICS.COURSE_TYPE_COMPONENT_MASTER b where a.STDNTSLGNDTLS_REGISTER_NUMBER =?1 and a.COURSE_OPTION_MASTER_CODE "+
	...
	List<Object[]> findByRegisterNumberCourseOptionAndGrade(String registerNumber, List<String> courseOptionCode, List<String> grade);
	...
	//Student History
	@Query(value="select TOTAL_CREDITS_REGISTERED, TOTAL_CREDITS_EARNED, CUMULATIVE_GRADE_POINT_AVERAGE, "
			+ " NUMBER_OF_S_GRADE, NUMBER_OF_A_GRADE,NUMBER_OF_B_GRADE, NUMBER_OF_C_GRADE, NUMBER_OF_D_GRADE, NUMBER_OF_E_GRADE,  "
			+ " NUMBER_OF_F_GRADE, NUMBER_OF_N_GRADE,STDNTSLGNDTLS_REGISTER_NUMBER from examinations.STUDENT_CGPA_HISTORY a where STDNTSLGNDTLS_REGISTER_NUMBER =?1  "
			+ " and LOCK_STATUS =0 and MODIFIED_TIMESTAMP = ( "
			+ " select max(MODIFIED_TIMESTAMP) from examinations.STUDENT_CGPA_HISTORY where STDNTSLGNDTLS_REGISTER_NUMBER=a.STDNTSLGNDTLS_REGISTER_NUMBER  "
			+ " and LOCK_STATUS =0) ",nativeQuery=true)
	List<Object[]> getStaticStudentCGPAFromTable(String regNoList);


	@Query(value="select a.CRSTYPCMPNTMASTER_COURSE_TYPE as GEN_COURSE_TYPE from academics.STUDENT_HISTORY a, "
		+ "academics.COURSE_TYPE_COMPONENT_MASTER b  " + 
		"where a.STDNTSLGNDTLS_REGISTER_NUMBER =?1 and a.GRADE in ('Fail','F','Y','N','N1','N2','N3','N4')  " + 
		"and a.CRSTYPCMPNTMASTER_COURSE_TYPE=b.COURSE_TYPE and a.course_catalog_course_id=?2 " + 
		"and a.CRSTYPCMPNTMASTER_COURSE_TYPE not in ('ETL','ETLP')" ,nativeQuery=true)
	List<String> doGetFaildCourseByRegNoAndCourseId(String regNo,String courseId);

	
	@Query(value="select a.CRSTYPCMPNTMASTER_COURSE_TYPE as GEN_COURSE_TYPE from academics.STUDENT_HISTORY a, "
		+ "academics.COURSE_TYPE_COMPONENT_MASTER b  " + 
		"where a.STDNTSLGNDTLS_REGISTER_NUMBER =?1 and a.GRADE not in ('Fail','F','Y','N','N1','N2','N3','N4')  " + 
		"and a.CRSTYPCMPNTMASTER_COURSE_TYPE=b.COURSE_TYPE and a.course_catalog_course_id=?2 " + 
		"and a.CRSTYPCMPNTMASTER_COURSE_TYPE not in ('ETL','ETLP')" ,nativeQuery=true)
	List<String> doGetPassedCourseByRegNoAndCourseId(String regNo,String courseId);
	
	@Query(value="select distinct b.code  from academics.course_registration a  " + 
			"inner join academics.course_equivalance_reg c  " + 
			"on c.semstr_details_semester_sub_id =a.semstr_details_semester_sub_id  " + 
			"and a.stdntslgndtls_register_number  = c.stdntslgndtls_register_number  " + 
			"and a.course_catalog_course_id  = c.course_catalog_course_id  " + 
			"and a.crstypcmpntmaster_course_type  = c.crstypcmpntmaster_course_type  " + 
			"inner join academics.course_catalog b " + 
			"on c.equivalance_course_id  = b.course_id " + 
			"where   a.stdntslgndtls_register_number =?1 " + 
			"and SUBSTRING (a.course_option_master_code, 1, 2) ='CS' ",nativeQuery=true)
	List<String> doGetCSCourseFilter(String registerNumber);

	@Query(value="select  COURSE_CATALOG_COURSE_ID, COURSE_CODE, COURSE_TITLE, CRSTYPCMPNTMASTER_COURSE_TYPE, " + 
			" to_char(EXAM_MONTH,'DD-MON-YYYY') as exam_date from academics.STUDENT_HISTORY where  " + 
			"STDNTSLGNDTLS_REGISTER_NUMBER =?1 and COURSE_CODE not in (?5)  " + 
			"and GRADE in ('Fail','F','N','N1','N2','N3','N4')  " + 
			"and CRSTYPCMPNTMASTER_COURSE_TYPE not in ('ETH','ELA','EPJ','PJT','SS','OC','ECA') " + 
			"and course_code  in (select c.CODE  from ACADEMICS.PRG_SPLZTN_CURRICULUM_DETAILS a, ACADEMICS.BASKET_COURSE_CATALOG b,  " + 
			"ACADEMICS.COURSE_CATALOG c where a.PRGSPLZN_PRG_SPECIALIZATION_ID=?2 and a.ADMISSION_YEAR=?3  " + 
			"and a.CURRICULUM_VERSION=?7 and a.CATALOG_TYPE='BC' and a.COURSE_CATEGORY  in (?6) and  " + 
			"a.LOCK_STATUS=0 and a.COURSE_BASKET_ID=b.BASKET_DETAILS_BASKET_ID and  " + 
			"a.COURSE_BASKET_ID = ?4 AND " + 
			"b.COURSE_CATALOG_COURSE_ID=c.COURSE_ID) ",nativeQuery=true)
	List<Object[]> doGetBasketCourseForCSByRegNoAndSpecIdAndAdmYrAndBsktIdAndCrsCodeAndCrsCatgAndCurVersion(String regNo,int specId,int admissionsYear,
			String basketId,String courseCode,String crsCatg,Float curVersion);
	...
	@Query(value="select COURSE_CATALOG_COURSE_ID, COURSE_CODE, COURSE_TITLE, CRSTYPCMPNTMASTER_COURSE_TYPE, "+
			...
			"(select b.CODE from ACADEMICS.PRG_SPLZTN_CURRICULUM_DETAILS a, ACADEMICS.COURSE_CATALOG b "+
			"where a.PRGSPLZN_PRG_SPECIALIZATION_ID=?3 and a.ADMISSION_YEAR=?4 and a.CURRICULUM_VERSION=?5 "+
			"and a.CATALOG_TYPE='CC' and a.COURSE_CATEGORY not in (?6) and a.LOCK_STATUS=0 and "+
			"a.COURSE_BASKET_ID=b.COURSE_ID) and COURSE_CODE not in "+
			"(select c.CODE from ACADEMICS.PRG_SPLZTN_CURRICULUM_DETAILS a, ACADEMICS.BASKET_COURSE_CATALOG b, "+
			"ACADEMICS.COURSE_CATALOG c where a.PRGSPLZN_PRG_SPECIALIZATION_ID=?3 and a.ADMISSION_YEAR=?4 "+
			"and a.CURRICULUM_VERSION=?5 and a.CATALOG_TYPE='BC' and a.COURSE_CATEGORY not in (?6) and "+
			"a.LOCK_STATUS=0 and a.COURSE_BASKET_ID=b.BASKET_DETAILS_BASKET_ID and "+
			"b.COURSE_CATALOG_COURSE_ID=c.COURSE_ID) order by COURSE_CODE", nativeQuery=true)
	List<Object[]> findStudentHistoryCSDEAndOE(String registerNumber, List<String> courseCode, Integer specializationId, 
			Integer studentYear, Float curriculumVersion,String courseCategory);
