...
public interface SemesterMasterRepository extends JpaRepository<SemesterMasterModel, Integer>
{
	...
	@Query(value="select distinct crsEq.equv_course_id,  crsEq.equv_course_code,  cc.course_id, cc.code, " + 
			"eqcc.generic_course_type as equv_generic_course_type, cc.generic_course_type, " + 
			"to_char(current_date,'DD-MON-YYYY') as hist_date, eqcc.course_system as equv_course_system,   " + 
			"cc.course_system from (select distinct a.equv_course_id, a.equv_course_code, a.course_catalog_course_id,a.course_code  from ( " + 
			"(select equivalent_course_id as equv_course_id, equivalent_course_code as " + 
			"equv_course_code, course_catalog_course_id, course_code  from academics.course_equivalances " + 
			"where equivalent_course_code=?1 and " + 
			"course_code<>equivalent_course_code)) a) crsEq " + 
			"inner join academics.course_catalog  eqcc on (eqcc.code=crsEq.equv_course_code) " + 
			"inner join academics.course_catalog  cc on (cc.code=crsEq.course_code) " + 
			"order by equv_course_id, cc.course_id desc", nativeQuery=true)
	...
	@Query(value=" select course_id ,ctcm.crs_ctlg_generic_course_type,"
			+ "lecture_credits ,practical_credits ,project_credits ,credits     "
			+ "from academics.course_catalog cc2,   "
			+ "academics.course_type_master ctcm where code in (  "
			+ "select corequisite  from academics.course_catalog cc where  "
			+ "course_id =?1) and   "
			+ "cc2.generic_course_type  = ctcm.crs_ctlg_generic_course_type ", nativeQuery = true)
	List<Object[]> getCoRequisiteCourseTypeInformation(String courseId);
	//Bridge Course Condition Detail
	@Query(value="select (case when (result is null) then 'NONE' else result end) as ept_result from "+
					"ACADEMICS.STUDENT_EPT_DETAILS where STDNTSLGNDTLS_REGISTER_NUMBER =?1 ", nativeQuery=true)
	String findEPTResultByRegisterNumber(String registerNumber);
	
	@Query(value="select (case when (HSC_GROUP is null) then 'NONE' else HSC_GROUP end) as hsc_grp from "+
					"ACADEMICS.STUDENT_BRIDGE_COURSE_DETAIL where REGNO = ?1 ", nativeQuery=true)
	String findPCMBStatusByRegisterNumber(String registerNumber);
	...
	@Query(value="select MARK, STUDENT_SEMESTER from ACADEMICS.STUDENT_SPT_DETAILS where "+
					"STDNTSLGNDTLS_REGISTER_NUMBER = ?1 and STUDENT_SEMESTER=?2", nativeQuery=true)
	List<Object[]> findSPTResultByRegisterNumberAndSemester(String registerNumber, Integer studentSemester);
		
	@Query(value="select MARK, RESULT from ACADEMICS.STUDENT_EPT_DETAILS where STDNTSLGNDTLS_REGISTER_NUMBER = ?1 ", 
					nativeQuery=true)
	List<Object[]> findEPTDetailByRegisterNumber(String registerNumber);
	...
	@Query(value="select a.REG_NO as registerNumber, b.OLD_REGISTER_NUMBER as oldRegisterNumber, "+ 
					"(case when (c.stdntslgndtls_register_number is null) then 0 else 1 end) as examGraduationStatus, "+ 
					"(case when (d.lock_status is null) then 0 else d.lock_status end) as registrationExcemptionStatus, "+ 
					"f.mark as eptMark, g.mark as sptMark, h.hsc_group as hscGroup "+ 
					"from (select REG_NO, prgsplprgrm_specialization_id from ADMISSIONS.STUDENTS_LOGIN_DETAILS where REG_NO=?1) a "+ 
					"left outer join (select STDNTSLGNDTLS_REGISTER_NUMBER, OLD_REGISTER_NUMBER from ACADEMICS.STUDENT_CREDIT_TRANSFER "+ 
					"where STDNTSLGNDTLS_REGISTER_NUMBER=?1) b on (b.STDNTSLGNDTLS_REGISTER_NUMBER=a.REG_NO) "+ 
					"left outer join (select a.stdntslgndtls_register_number from ("+ 
					"(select a.stdntslgndtls_register_number from examinations.graduate_student a where a.STDNTSLGNDTLS_REGISTER_NUMBER=?1 "+ 
					"and NOT exists (SELECT b.stdntslgndtls_register_number FROM examinations.graduate_student b where "+ 
					"b.Stdntslgndtls_Register_Number like '%MBI%' and b.programme_specialization_id=27 and "+ 
					"b.stdntslgndtls_register_number=a.Stdntslgndtls_Register_Number)) UNION ALL "+ 
					"(select stdntslgndtls_register_number from examinations.graduate_research_scholar where "+ 
					"STDNTSLGNDTLS_REGISTER_NUMBER=?1)) a) c on (c.stdntslgndtls_register_number=a.REG_NO) "+ 
					"left outer join (select STDNTSLGNDTLS_REGISTER_NUMBER, lock_status from ACADEMICS.REGISTRATION_EXEMPTION_DETAIL "+ 
					"where SEMSTR_DETAILS_SEMESTER_SUB_ID=?2 and STDNTSLGNDTLS_REGISTER_NUMBER=?1) d on (d.STDNTSLGNDTLS_REGISTER_NUMBER=a.REG_NO) "+ 
					"left outer join (select * from ACADEMICS.student_ept_details where STDNTSLGNDTLS_REGISTER_NUMBER=?1) f "+ 
					"on (f.stdntslgndtls_register_number=a.REG_NO) "+ 
					"left outer join (select * from ACADEMICS.STUDENT_SPT_DETAILS where STDNTSLGNDTLS_REGISTER_NUMBER=?1) g "+ 
					"on (g.stdntslgndtls_register_number=a.REG_NO) "+ 
					"left outer join (select * from academics.student_bridge_course_detail where regno=?1) h "+ 
					"on (h.regno=a.REG_NO)", nativeQuery= true)
	List<Object[]> findStudentDetailOthersByRegisterNumberAndSemesterSubId(String registerNumber, String semesterSubId);

	@Query(value="select distinct a.CODE, a.DESCRIPTION,c.code as courseCode, c.course_id ,c.credits ,c.generic_course_type "
			...
			+ " and a.code in (?4) "+
			"and a.LOCK_STATUS=0 and a.CODE=b.ADDTNL_LEARNING_DETAILS_CODE and b.LOCK_STATUS=0 and "+
			"b.COURSE_CATALOG_COURSE_ID=c.COURSE_ID  order by a.CODE", nativeQuery=true)
	List<Object[]> getAdditionalLearningDetailsForCBCSMinor(String learningType, Integer groupId, String specId, List<String> cbcsMinorList);


	@Query(value="select distinct a.CODE from academics.additional_learning_details a inner join academics.additional_learning_crs_ctlg b"
			+" on a.code  = b.addtnl_learning_details_code and "
			+" a.learning_system =?1 and a.learning_type =?2 and a.lock_status =?3 "
			+" and b.lock_status =?3 order by a.code ",nativeQuery = true)
	List<String> doGetAdditionalLearningCode(String learningSystem,String learningType,int lockStatus);
