...
{		
	...
	@Query(value="select a.SEMSTR_DETAILS_SEMESTER_SUB_ID, a.COURSE_CATALOG_COURSE_ID, a.CRSTYPCMPNTMASTER_COURSE_TYPE, "+
					...
					"=?2 and a.COURSE_OPTION_MASTER_CODE in (?3) and a.SEMSTR_DETAILS_SEMESTER_SUB_ID not in (?1) and "+
					...
					"and a.STDNTSLGNDTLS_REGISTER_NUMBER =?2 and a.SEMSTR_DETAILS_SEMESTER_SUB_ID=b.SEMSTR_DETAILS_SEMESTER_SUB_ID and "+
					"a.STDNTSLGNDTLS_REGISTER_NUMBER=b.STDNTSLGNDTLS_REGISTER_NUMBER and "+
					"a.COURSE_CATALOG_COURSE_ID=b.COURSE_CATALOG_COURSE_ID and "+
					"a.CRSTYPCMPNTMASTER_COURSE_TYPE=b.CRSTYPCMPNTMASTER_COURSE_TYPE) "+
					...
					nativeQuery=true)
	List<Object[]> findByRegisterNumberAndCourseCode(String semesterSubId, String registerNumber, List<String> courseOptionCode, 
						String courseCode);
	
	@Modifying
	@Query(value="INSERT INTO ACADEMICS.ERROR_LOG (EXCEPTION_MESSAGE, ERROR_DATETIME, VERTICAL_NAME, "+
			"PACKAGE_NAME, PROGRAMME_NAME, LOG_USERID, LOG_IPADDRESS,exception_trace) VALUES (?1, clock_timestamp(), "+
			"?2, ?3, ?4, ?5, ?6,?7)", nativeQuery=true)
	void InsertErrorLog(String exceptionMessage, String verticalName, String packageName, String programName, 
			String userId, String ipAddress,String exceptionTrace);
